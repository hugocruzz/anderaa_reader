"""
Aanderaa Sensor Communication Script with Config File Support
"""
...
def load_config(config_path: str = "sensor_config.json") -> List[SensorConfig]:
    ...
    try:
        ...
        for sensor_data in config_data['sensors']:
            config = SensorConfig(
                ...
                com_port=sensor_data['com_port'],
                baudrate=sensor_data.get('baudrate', 9600),
                timeout=sensor_data.get('timeout', 2),
...
def main():
    ...
    if not sensor_configs:
        logger.error("No sensor configurations loaded. Exiting.")
        return
